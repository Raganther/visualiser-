// Journey choice test: over many simulated sections with random music, every world, hit and (nearly) every recipe
// gets chosen, and hits appear in a sensible share of sections. Runs on index.html (reads the modules).
...
const r = await page.evaluate(async (N) => {
  ...
  const {presets} = await import('/src/presets.js'), reg = await import('/src/visuals/registry.js');
  __step(60*12);                                       // let the first section form
  const worlds = {}, hits = {}, recipes = {};
  for (let i = 0; i < N; i++) {
