...
export const LAYER_VISUALS = [ring, scope, plasma, burst, comets, flow, ribbons, horizon];
...
export const ELEMS = LAYER_VISUALS.map(v => v.key);
...
export const SUITS = Object.fromEntries(LAYER_VISUALS.map(v => [v.key, v.suits]));
export const OVER_WORLD = Object.fromEntries(LAYER_VISUALS.filter(v => v.overWorld !== undefined).map(v => [v.key, v.overWorld]));
export const ACCENT = Object.fromEntries(LAYER_VISUALS.map(v => [v.key, v.accent]));
export const ALT_TRIG = Object.fromEntries(LAYER_VISUALS.filter(v => v.altAccent).map(v => [v.key, v.altAccent]));
